...
export function buildRankTrends(players, matches, windowSize = 3) {
  const chronological = [...matches].sort((a, b) => new Date(a.date) - new Date(b.date) || a.id.localeCompare(b.id));
  const previous = buildLeaderboard(players, chronological.slice(0, Math.max(0, chronological.length - windowSize)));
  const current = buildLeaderboard(players, chronological);
  const previousRanks = new Map(previous.map((player, index) => [player.id, index + 1]));
  return Object.fromEntries(current.map((player, index) => {
